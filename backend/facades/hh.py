...
class HHFacade:
    ...
    async def authorize(self, username: str, password: str) -> HHAuthTokenResponse:
        """
        ...
        response = await self.client.post("/oauth/token", data=data, headers=headers)
    ...
    async def list_ranked_vacancies(self, access_token: str, limit: int = 20, page: int = 0) -> list[HHVacancyResponse]:
        """
        ...
        response = await self.client.get("/vacancies", headers=headers, params=params)
        response.raise_for_status()
        data = response.json()
